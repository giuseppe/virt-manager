...
def list_os(list_types=False, typename=None,
    ...
    for key, osinfo in _allvariants.items():
        ...
        if typename and typename != osinfo.typename:
            continue
        if filtervars:
            filtervars = map(lambda x: lookup_os(x).name, filtervars)
...
def get_minimum_resources(variant, arch):
    v = _allvariants.get(variant)
    if v is None:
        return None

    return v.get_minimum_resources(arch)
...
class _OSVariant(object):
    ...
    def get_minimum_resources(self, arch):
        ignore1 = arch
        return None
